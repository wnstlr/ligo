...
if __name__ == '__main__':
    ...
    f = numpy.linspace(20, 2048, l)
    f_matlab = numpy.linspace(0, 2048, len(imr_amp))
    ...
    pylab.loglog(f, IMR_amp, 'r', alpha=0.5, label="From Python")
    pylab.loglog(f_matlab, imr_amp, 'b', alpha=0.3, label="From MATLAB")
    pylab.xlabel("Frequency (Hz)")
    pylab.ylabel("Amplitude $|A(f)|$")
    pylab.title("Insipiral Merger Ringdown Amplitude")
    pylab.legend(loc="upper right", prop={'size':10})
    pylab.xlim(10, 2 * 10 ** 3)
    pylab.grid(True)
    pylab.show()
