...
'''
Tests with PN/IMR template for GR and scale/graviton template for nonGR.
Chi2 Test and Kolmogorov-Smirnov Test implemented for oberserving Deviation.
Improved the GR signal with Insipiral Merger Ringdown (IMR) signal.
...
@ Test type: chi2 (Chi-square Test), ks (Kolmogorov-Smirnov Test)
-------------------------------------------------------------------------------
'''
...
if __name__ == '__main__':
    ...
    print("Select " + str(testtype_options))
    testtype = raw_input("Test type: ")
    while (testtype not in testtype_options):
        testtype = raw_input("Try again: ")
    ### Degrees of freedom
    N = 2
    ...
    grav_list = numpy.logspace(12, 30, 19) 
    ...
    * (numpy.pi * mc * f_pn) ** (-5./3.)
    ...
    for j in range(len(f_imr)):
        signal_mg_amp[j] = computeIMRChirpAmplitudeFD(f_imr[j],f1,f2,f3,sigma,\
                                                  ...
                                                  epsilon1,epsilon2,m,w_m,w_r)
    ### When the test type is chi2 test
    if (testtype == 'chi2'):
        ...
        if (modtype == 'scaledGR'):
            for i in range(len(epsil)):
                ...
                theta.append(s)
            numpy.savetxt("scaled_prob.txt", theta)
        ...
        ### When signal buried follows massive graviton theory
            ...
            x_current = numpy.log10(numpy.array([2.8e15, 2.8e15]))
            ...
            for i in range(len(beta_list)):
                ...
                for j in range(count,len(n)):
                    s = s + ((bins[j+1] - bins[j]) * n[j])
                theta.append(s)
            numpy.savetxt("graviton_prob.txt", theta)
            pylab.plot(numpy.log10(grav_list), theta, "g")
            pylab.plot(x_current, y_current, "k--")
            pylab.xlabel("Mass of the Graviton in Seconds")
            pylab.ylabel("Efficiency of Detecting Deviations from GR")
            ...
            #pylab.close()
    ### When the test type is Kolmogorov-Smirnov Test
    elif (testtype == "ks"):
        K_alpha = findKAlpha(fap)
        ### When signal buried has the form of (1 + epsil) * signal_GR 
        if (modtype == "scaledGR"):
            result = []
            for i in range(len(epsil)):
                ck = (nk + epsil[i] * signal_GR)
                pk = (ck.real ** 2 + ck.imag ** 2) / (sigmak ** 2)
                print(">> Epsilon = " + str(epsil[i]) + " : ")
                n, bins, patches = pylab.hist(pk, 500, normed=True)
                pylab.close()
                use_bins = numpy.linspace(bins[0], bins[500], l)
                D, p = kstest(pk, "chi2", args=(2, 0, 1))
                print("P-value = " + str(p))
                if (p >= fap):
                    print("Follows chi2 distribution with 2 DF with confidence level of "\
                        + str(fap * 100) + " percent.")
                else:
                    print("Does NOT follow chi2 disbribution with confidence level of "\
                        + str(fap * 100) + " percent.")

        ### When signal buried follows Massive-Graviton template
        elif (modtype == "graviton"):
            result = []
            for i in range(len(beta_list)):
                Graviton_Psi = IMR_Psi - beta_list[i] * 1./ f_imr + phi_g + tau_g * f_imr
                ...
                pylab.close()
                use_bins = numpy.linspace(bins[0], bins[500], l)
                print(">> Graviton Wavelength (m) = " + str(grav_list[i]) + " : ")
                D, p = kstest(pk, "chi2", args=(2, 0, 1))
                print("P-value = " + str(p))
                ###########
                if (p >= fap):
