...
if __name__ == '__main__':
    ...
    f = numpy.linspace(f0, f_isco, l) # frequency domain 
    epsil = [0, 0.01, 0.1, 0.5, 0.75] # list of degrees of deviation
    S_h = generatePSD(f) # create PSD
    nk = generateNoise(S_h) # create noise following PSD
    h_GR = computeNewtonianChirpFD(d, mc, f, phi0, t0) # create chirp signal in f
    SNR = numpy.sqrt(4 * numpy.sum((h_GR.real ** 2 + h_GR.imag ** 2) / S_h * \
                                       ((f_isco - f0) / l )))
    sigmak = 2 * numpy.sqrt(S_h) # estimate standard deviation
    ...
    pylab.plot(bins, y, "k--", linewidth=1.0)
    ...
    pylab.figtext(0.5,0.5,'SNR = ' + str(SNR), fontsize=10)
    pylab.xlim(0, 1)
    pylab.ylim(0, 2.8)
    pylab.show()
    #pylab.savefig("General_Noise_Deviation.pdf")
    #pylab.close()
